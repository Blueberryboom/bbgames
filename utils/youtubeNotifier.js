...
module.exports = {
  initYouTubeNotifier(client) {
    ...
    runCheck(client).catch(err => {
      console.error('⚠️ Initial YouTube notifier check failed:', err);
    });

    const interval = setInterval(() => {
      runCheck(client).catch(err => {
        console.error('⚠️ YouTube notifier check failed:', err);
      });
    }, CHECK_INTERVAL_MS);
  ...
  try {
    ...
    for (const sub of subscriptions) {
      try {
        ...
        console.error(`⚠️ YouTube notifier failed for ${sub.youtube_channel_id} in guild ${sub.guild_id}:`, err.message || err);
      }
    }
  } finally {
