...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (!await checkPerms(interaction)) {
      return interaction.reply({ content: '⚠️ You need administrator or configured bot manager role.', flags: MessageFlags.Ephemeral });
    }
    ...
    if (sub === 'channel') {
      ...
      if (disabledAt && reenableAt > Date.now()) {
        return interaction.reply({
          content: `⚠️ Bumping was recently disabled. You can enable it again <t:${Math.floor(reenableAt / 1000)}:R> (at <t:${Math.floor(reenableAt / 1000)}:F>).`,
      ...
      if (!everyonePerms?.has('ViewChannel')) {
        return interaction.reply({ content: '⚠️ The bumping channel must be visible to everyone in the server!', flags: MessageFlags.Ephemeral });
      }
    ...
    if (sub === 'verification') {
      ...
      if (bumpCount < 50) {
        return interaction.reply({
          content: `⚠️ Verification requires at least **50** bumps. This server currently has **${bumpCount}**.`,
          flags: MessageFlags.Ephemeral
        });
      }
      ...
      if ((interaction.guild?.memberCount || 0) < 100) {
        return interaction.reply({
          content: '⚠️ Verification requires at least **100** server members.',
          flags: MessageFlags.Ephemeral
        });
      }
      ...
      if (lastRequestAt > 0 && requestAvailableAt > Date.now()) {
        return interaction.reply({
          content: `⚠️ You can send another verification request <t:${Math.floor(requestAvailableAt / 1000)}:R> (at <t:${Math.floor(requestAvailableAt / 1000)}:F>).`,
          flags: MessageFlags.Ephemeral
        });
      }
    ...
    if (ad.split('\n').length > 10) {
      return submit.reply({ content: '⚠️ Advertisement cannot exceed 10 lines.', flags: MessageFlags.Ephemeral });
    }
    if (hasBlockedText(ad)) {
      return submit.reply({ content: '⚠️ Advertisement contains blocked content (links, mentions, or offensive text).', flags: MessageFlags.Ephemeral });
    }
