  ...
  async execute(interaction) {
    ...
    if (sub === 'configure') {
      ...
      if (!/^[a-z0-9-_]{2,40}$/.test(rawName)) {
        return interaction.reply({ content: '❌ Name must be 2-40 chars and use lowercase letters, numbers, dashes, or underscores.', flags: MessageFlags.Ephemeral });
      }

      const normalizedEmoji = normalizeEmojiInput(rawEmoji);
      if (!normalizedEmoji) {
        return interaction.reply({ content: '❌ Invalid emoji input. Use a Unicode emoji or custom emoji like <:name:id>.', flags: MessageFlags.Ephemeral });
      }

      const embedColor = colorInput ? parseHexColor(colorInput) : null;
      if (colorInput && embedColor == null) {
        return interaction.reply({ content: '❌ Color must be a valid hex code such as #191919 or 191919.', flags: MessageFlags.Ephemeral });
      }
      ...
      if (!exists && total >= limit) {
        return interaction.reply({ content: `❌ Starboard limit reached (${limit}).`, flags: MessageFlags.Ephemeral });
      }
    ...
    if (sub === 'remove') {
      ...
      if (!configRows.length) {
        return interaction.reply({ content: '❌ Starboard config not found.', flags: MessageFlags.Ephemeral });
      }
