  ...
  async execute(interaction) {
    ...
    if (sub === 'config') {
      ...
      if (cooldownMs == null || cooldownMs < 2 * 60 * 1000) {
        return interaction.reply({ content: '⚠️ Cooldown must be a valid duration and greater than 2 minutes.', flags: MessageFlags.Ephemeral });
      }
    ...
    if (sub === 'disable') {
      ...
      if (durationMs == null || durationMs < 2 * 60 * 1000) {
        return interaction.reply({ content: '⚠️ Time must be a valid duration and greater than 2 minutes.', flags: MessageFlags.Ephemeral });
      }

      const exists = await query('SELECT guild_id FROM suggestion_settings WHERE guild_id = ? LIMIT 1', [interaction.guildId]);
      if (!exists.length) {
        return interaction.reply({ content: '⚠️ Suggestions are not configured yet.', flags: MessageFlags.Ephemeral });
      }
    ...
    if (sub === 'delete_category') {
      ...
      if (!result.affectedRows) {
        return interaction.reply({ content: '⚠️ Category not found.', flags: MessageFlags.Ephemeral });
      }
