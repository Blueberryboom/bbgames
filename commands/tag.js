  ...
  async execute(interaction) {
    ...
    if (sub === 'create') {
      ...
      if (!/^[a-z0-9-]{2,40}$/.test(rawName)) {
        return interaction.reply({
          content: '⚠️ Tag names must be 2-40 characters and use only lowercase letters, numbers, and dashes.',
          flags: MessageFlags.Ephemeral
        });
      }
      ...
      if (!exists && totalTags >= limit) {
        return interaction.reply({
          content: `⚠️ Tag limit reached for this server (${limit}).`,
          flags: MessageFlags.Ephemeral
        });
      }
      ...
      if (usableBy === 'roles') {
        ...
        if (!roleIds.length) {
          return interaction.reply({
            content: '⚠️ When `usable_by` is `Specific roles`, provide role IDs in `allowed_roles`.',
            flags: MessageFlags.Ephemeral
          });
        }
        ...
        for (const roleId of roleIds) {
          if (!interaction.guild.roles.cache.has(roleId)) {
            return interaction.reply({
              content: `⚠️ Invalid role ID: \`${roleId}\``,
              flags: MessageFlags.Ephemeral
            });
          }
        }
    ...
    if (sub === 'delete') {
      ...
      if (!result.affectedRows) {
        return interaction.reply({
          content: '⚠️ That tag does not exist.',
          flags: MessageFlags.Ephemeral
        });
      }
    ...
    if (!await canUseTag(interaction, tag)) {
      return interaction.reply({
        content: '⚠️ You do not have permission to use this tag.',
        flags: MessageFlags.Ephemeral
      });
    }
