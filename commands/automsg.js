  ...
  async execute(interaction) {
    ...
    if (sub === 'create') {
      ...
      if (missingPermissions.length) {
        return interaction.reply({
          content: `⚠️ I can't send automatic messages in ${channel}. Missing: ${formatPermissionList(missingPermissions)}.`,
          flags: MessageFlags.Ephemeral
        });
      }
      ...
      if (!intervalMs) {
        return interaction.reply({
          content: '⚠️ Invalid interval format. Use combinations like `2h 5m`, `4m 36s`, or `2h 5s`.',
          flags: MessageFlags.Ephemeral
        });
      }
      ...
      if (intervalMs < MIN_INTERVAL_MS || intervalMs > MAX_INTERVAL_MS) {
        return interaction.reply({
          content: '⚠️ Interval must be between 30 minutes and 24 hours.',
          flags: MessageFlags.Ephemeral
        });
      }
      ...
      if (currentRows.length >= limit) {
        return interaction.reply({
          content: `⚠️ This bot can only have ${limit} automatic messages in this server.`,
          flags: MessageFlags.Ephemeral
        });
      }
    ...
    if (sub === 'remove') {
      ...
      if (!rows.length) {
        return interaction.reply({
          content: '⚠️ Auto message not found for this server.',
          flags: MessageFlags.Ephemeral
        });
      }
