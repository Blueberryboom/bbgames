  ...
  async execute(interaction) {
    if (!await checkPerms(interaction, { scope: 'giveaway' })) {
      return interaction.reply({
        content: '⚠️ You need administrator, the bot manager role, or the giveaway admin role to use this command.',
    ...
    if (sub === 'create') {
      ...
      if (winners < 1 || winners > 20) {
        return interaction.reply({ content: '⚠️ Winners must be between 1 and 20.', flags: MessageFlags.Ephemeral });
      }
      ...
      try {
        ...
        return interaction.reply({
          content: '⚠️ Invalid duration. Example formats: `30m`, `2h`, `1d 2h 30m`.',
          flags: MessageFlags.Ephemeral
        });
      }
