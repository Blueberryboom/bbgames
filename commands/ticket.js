...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'close_request') {
      ...
      if (!ticket) {
        return interaction.reply({ content: '⚠️ This command can only be used inside an open ticket channel.', flags: MessageFlags.Ephemeral });
      }
      ...
      if (!isStaff) {
        return interaction.reply({ content: '⚠️ Only assigned ticket staff or administrators can send close requests.', flags: MessageFlags.Ephemeral });
      }
    ...
    if (sub === 'unclaim') {
      ...
      if (!ticket.claimed_by) {
        return interaction.reply({ content: '⚠️ This ticket is not currently claimed.', flags: MessageFlags.Ephemeral });
      }
      ...
      if (!isStaff && !isClaimer) {
        return interaction.reply({ content: '⚠️ Only the claimer, ticket staff, or admins can unclaim this ticket.', flags: MessageFlags.Ephemeral });
      }
    ...
    if (sub === 'add_user') {
      ...
      const isStaff = await checkPerms(interaction, { scope: 'staff' }) || staffRoleIds.some(roleId => interaction.member.roles.cache.has(roleId));
      if (!isStaff) {
        return interaction.reply({ content: '⚠️ Only ticket staff can add users to a ticket.', flags: MessageFlags.Ephemeral });
      }
    ...
    if (sub === 'remove_user') {
      ...
      const isStaff = await checkPerms(interaction, { scope: 'staff' }) || staffRoleIds.some(roleId => interaction.member.roles.cache.has(roleId));
      if (!isStaff) {
        return interaction.reply({ content: '⚠️ Only ticket staff can remove users from a ticket.', flags: MessageFlags.Ephemeral });
      }

      if (target.id === ticket.user_id) {
        return interaction.reply({ content: '⚠️ You cannot remove the ticket owner from their own ticket.', flags: MessageFlags.Ephemeral });
      }
    ...
    if (!await checkPerms(interaction)) {
      return interaction.reply({
        content: '⚠️ You need administrator or the configured bot manager role to use ticket configuration commands.',
        flags: MessageFlags.Ephemeral
      });
    }
    ...
    if (sub === 'creation_cooldown') {
      ...
      if (cooldownMs === null) {
        return interaction.reply({
          content: '⚠️ Invalid format. Use values like `2d 9h 54m`, `2h`, `4m`, or `1d 2m`.',
          flags: MessageFlags.Ephemeral
        });
      }
    ...
    if (sub === 'panel') {
      ...
      if (!settings?.category_id) {
        return interaction.reply({
          content: '⚠️ Configure ticket settings first using `/ticket config`.',
          flags: MessageFlags.Ephemeral
        });
      }
      ...
      if (!types.length) {
        return interaction.reply({
          content: '⚠️ No ticket types exist yet. Create one with `/tickets create_type` first.',
          flags: MessageFlags.Ephemeral
        });
      }
    ...
    if (sub === 'delete_type') {
      ...
      if (!type) {
        return interaction.reply({ content: '⚠️ Ticket type not found.', flags: MessageFlags.Ephemeral });
      }
      ...
      if (openCount > 0) {
        return interaction.reply({
          content: `⚠️ Cannot delete **${type.name}** while it has open tickets (${openCount}).`,
          flags: MessageFlags.Ephemeral
        });
      }
