...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      console.error('⚠️ Birthday command error:', error);
      if (!interaction.replied && !interaction.deferred) {
        return interaction.reply({
          content: '⚠️ Birthday action failed. Please try again.',
...
async function handleConfig(interaction) {
  if (!await checkPerms(interaction)) {
    return interaction.reply({
      content: '⚠️ You need administrator, manager role, or owner access for this command.',
      flags: MessageFlags.Ephemeral
    });
  }
...
async function handleRegister(interaction) {
  ...
  if (!parsed) {
    return interaction.reply({
      content: '⚠️ Invalid date format. Use DD/MM (example: 05/11).',
      flags: MessageFlags.Ephemeral
    });
  }
  ...
  if (!configRows.length || !configRows[0].enabled) {
    return interaction.reply({
      content: '⚠️ Birthday module is not enabled in this server. Ask an admin to run `/birthday config` first.',
      flags: MessageFlags.Ephemeral
    });
  }
  ...
  if (!isAdmin && !isOwner && !hasAllowedRole) {
    return interaction.reply({
      content: '⚠️ You do not have access to register birthdays in this server.',
      flags: MessageFlags.Ephemeral
    });
  }
  ...
  if (existingRows.length) {
    ...
    if (Date.now() < nextAllowedAt) {
      ...
      return interaction.reply({
        content: `⚠️ You can only change your birthday every ${REGISTER_COOLDOWN_MONTHS} months. You can update it again ${discordTimestamp}.`,
        flags: MessageFlags.Ephemeral
      });
    }
  }
