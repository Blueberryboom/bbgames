...
async function createSuggestion(interaction, title, description, categoryName) {
  ...
  if (!settings) {
    return interaction.reply({ content: '❌ Suggestions are not configured in this server yet. Ask staff to run `/suggestions config` first.', flags: MessageFlags.Ephemeral });
  }
  ...
  if (blacklisted.length) {
    return interaction.reply({ content: '❌ You are blacklisted from creating suggestions in this server.', flags: MessageFlags.Ephemeral });
  }
  ...
  if (allowedRoles.length && !allowedRoles.some(roleId => interaction.member.roles.cache.has(roleId))) {
    return interaction.reply({ content: '❌ You do not have permission to create suggestions.', flags: MessageFlags.Ephemeral });
  }
  ...
  if (!channel?.isTextBased()) {
    return interaction.reply({ content: '❌ The configured suggestions channel is missing or no longer text-based.', flags: MessageFlags.Ephemeral });
  }
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (category) {
      ...
      if (!rows.length) {
        return interaction.reply({ content: '❌ That category does not exist. Ask staff to create it with `/suggestions create_category`.', flags: MessageFlags.Ephemeral });
      }
    }
