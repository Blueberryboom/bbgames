    ...
    if (sub === 'add') {
      try {
        ...
        if (!targetChannel || targetChannel.guildId !== interaction.guildId) {
          return interaction.reply({
            content: '⚠️ Please pick a valid channel from this server.',
            flags: MessageFlags.Ephemeral
          });
        }
        ...
        if (existingRows.length >= maxSubscriptions) {
          return interaction.reply({
            content: `⚠️ You can only configure up to ${maxSubscriptions} YouTube channels per server.`,
            flags: MessageFlags.Ephemeral
          });
        }
        ...
        if (!resolved) {
          return interaction.reply({
            content: '⚠️ Could not find that YouTube channel. Try @handle, channel ID (UC...), username, or URL.',
            flags: MessageFlags.Ephemeral
          });
        }
        ...
        if (alreadyConfigured) {
          return interaction.reply({
            content: '⚠️ That YouTube channel is already configured in this server.',
            flags: MessageFlags.Ephemeral
          });
        }
        ...
        return interaction.reply({ content: '⚠️ Could not prepare this YouTube setup.', flags: MessageFlags.Ephemeral });
      }
    }
