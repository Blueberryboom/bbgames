  ...
  async execute(interaction) {
    if (!await checkPerms(interaction)) {
      return interaction.reply({
        content: '⚠️ You need administrator or configured bot manager role.',
    ...
    if (sub === 'create') {
      if (role.permissions.has(PermissionFlagsBits.Administrator)) {
        return interaction.reply({
          content: '⚠️ Autorole cannot be a role with Administrator permissions.',
          flags: MessageFlags.Ephemeral
        });
      }
      ...
      if (rows.length >= maxRoles) {
        return interaction.reply({
          content: `⚠️ You can only configure up to ${maxRoles} autoroles on this server.`,
          flags: MessageFlags.Ephemeral
        });
      }
