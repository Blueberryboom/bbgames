...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'redeem') {
      if (!interaction.inGuild()) {
        return interaction.reply({
          content: '⚠️ `/premium redeem` must be used in the server where you want perks enabled.',
      ...
      try {
        ...
        return interaction.reply({
          content: `⚠️ Could not redeem premium perks: ${error.message}`,
          flags: MessageFlags.Ephemeral
        });
      }
    }
    ...
    if (interaction.inGuild()) {
      return interaction.reply({
        content: '⚠️ `/premium start|stop|status` only works in DMs.',
        flags: MessageFlags.Ephemeral
      });
    }
    ...
    if (!allowed) {
      return interaction.reply({
        content: '⚠️ You are not allowed to use premium instances.',
        flags: MessageFlags.Ephemeral
      });
    }
    ...
    try {
      ...
      if (hasGlobalInstance && interaction.user.id !== BOT_OWNER_ID) {
        return interaction.editReply('⚠️ You already have an active premium bot instance.');
      }
      ...
      return interaction.editReply(`⚠️ Failed to start premium instance: ${error.message}`);
    }
  }
};
